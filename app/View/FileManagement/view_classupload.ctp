    ...
    <div class="block-content">
        <table class="table table-striped form">
            ...
            <tr>
                ...
                <th width="12%">Klas</th>
                <td width="38%">
                    <?=$typedetails->class ?> (<?=$educationLevel?> <?=$typedetails->education_level_year?>)
                </td>
            </tr>
            <tr>
                <th>
                ...
                <td>
                    <a href="file_management/download/<?=$file['id']?>" tager="_blank"><?=$file['name']?> (<?=$file['origname']?>)</a>
                </td>
                <th>
                    Stamklas
                </th>
                <td><?= $typedetails->is_main_school_class ? 'Ja' : 'Nee' ?></td>
            </tr>
        </table>
    </div>
</div>
...
<script>
    ...
    function save(id,part){
        ...
        var jqxhr = $.post('/file_management/update/<?= $file['id']?>',
                data,
                function(response) {
                    Loading.hide();

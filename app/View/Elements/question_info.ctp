<table width="100%" class="table table-striped">
    ...
    foreach($question['question']['p_value'] as $pvalue) {
        ?>
        <tr>
            <th width="200"><?= __("P-waarde")?> <?=$pvalue['education_level_year']?> <?=$pvalue['education_level']['name']?></th>
            <td><?=number_format($pvalue['p_value'], 2);?></td>
            <td><?=$pvalue['p_value_count']?> <?= __("keer afgenomen")?></td>
            <td>
                <?

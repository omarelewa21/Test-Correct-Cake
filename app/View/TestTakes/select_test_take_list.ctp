...
foreach($test_takes as $test_take) {
    ...
    <tr>
        ...
        <td class="nopadding" width="100">
            <a href="#" class="btn highlight pull-right" onclick="TestTake.setSelectedTestTake(<?=$test_take['id']?>, '<?=$test_take['test']['name']?> op <?=date('d-m-Y H:i', strtotime($test_take['time_start']))?>');">

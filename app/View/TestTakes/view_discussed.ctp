    ...
    <? if ($take['test_take_status_id'] == 8) { ?>
        <div id="nav_pdf_button">
            ...
            <ul id="select_drop" class="ui-dropdown action-dropdown">
                ...
                <? if ($hasPdfAttachments) { ?>
                    <li><a href="#"
                           onclick="Loading.show();Popup.load('/tests/pdf_showPDFAttachment/<?= getUUID($take['test'], 'get') ?>', 1000)"><?= __("Toets pdf bijlagen") ?></a>
                    </li>
                <? } ?>
                <li><a href="#"
                       onclick="Popup.showPreviewAnswerModelTest('<?= $test_uuid ?>');"><?= __("Antwoord model") ?></a>
                </li>
                <li><a href="#"
                       onclick="Popup.showPreviewTestTakeAnswers('<?= $take_id ?>')"><?= __("Antwoorden") ?></a></li>
            </ul>
        </div>
        <?
        if (!empty($take['show_results']) && time() < strtotime($take['show_results'])) {
            ...
            <a href="#" class="btn white mr2"
               onclick="TestTake.closeShowResults('<?= $take_id ?>', <?= $take['show_grades'] ? 1 : 0 ?>);">
                <span class="fa fa-eye-slash mr5"></span>
                <?= __("Dichtzetten") ?>
            </a>
        <? } else { ?>
            <a href="#" class="btn white mr2"
               onclick="Popup.load('/test_takes/update_show_results/<?= $take_id ?>', 420);">
        ...
        <?php if ($normalization && $take['is_rtti_test_take'] == 1): ?>
            <a href="#" onclick="updateRTTI('<?= $take_id ?>')" title="Exporteren naar RTTI-Online"
               class="btn white mr2">Exporteren naar RTTI-Online</a>
        <?php endif; ?>
...
<?php if (isset($take['test_take_code']) && !empty($take['test_take_code'])) { ?>
    <div class="test-take-code-show-wrapper">
        <div class="test-take-code-text-container">
            ...
            <h2 title="<?= __('Kopieer toetslink') ?>" onclick="TestTake.copyDirectlink('<?= $take['directLink'] ?>');"
                style="margin-left:1.5rem; color:#041f74; cursor:pointer;">
                ...
                <tr>
                    ...
                    <td class="nopadding" width="100">
                        <a href="#" class="btn white pull-right"
                           onclick="Navigation.load('/test_takes/view_results/<?= getUUID($take, 'get'); ?>/<?= getUUID($participant, 'get'); ?>');">
                            <span class="fa fa-folder-open-o"></span>
                        </a>
                        <a href="#" class="btn white pull-right"
                           onclick="Popup.load('/test_takes/rated_info/<?= getUUID($take, 'get'); ?>/<?= getUUID($participant, 'get'); ?>', 500);">
                            <span class="fa fa-info-circle"></span>
                        </a>
                        <a href="#" class="btn white pull-right"
                           onclick="Popup.load('/messages/send/<?= getUUID($participant['user'], 'get'); ?>', 500);">
